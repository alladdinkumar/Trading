...
| ~~F-027~~ | DEBT | Low | 6 | ~~Brittle 3-way coupling on the `### SYM — passes N/M rules` heading (context renderer / pre_open_iep rewrite / briefing regex), no spanning test~~ | ✅ Fixed 2026-06-29 — single source of truth in `llm.context` (`CANDIDATE_HEADING_FMT`/`_RE`/`render_candidate_heading`); briefing + pre_open_iep share the one compiled regex (object identity); spanning test `tests/test_candidate_heading_contract.py` |
| ~~F-028~~ | INACC | Low | 6 | ~~`assemble_context` docstring omits the sector + Layer-B ranker sections (added Phases 12.6/16)~~ | ✅ Fixed 2026-06-29 — docstring now lists all rendered sections in order (header, macro, sector momentum, candidates, Layer-B ranker, holdings health, open trades, post_close matured predictions) |
...
| ~~F-030~~ | DEBT | Low | 7 | ~~Visibility-only (non-selected) signals inserted unconditionally each pre_open run → duplicate `signals` rows on re-run~~ | ✅ Fixed 2026-06-29 — migration v8 partial UNIQUE `idx_signals_visibility_dedup(symbol, ts) WHERE created_by='pre_open'` + `insert_signal(or_ignore=True)`; scoped so selected/open_fills rows are unaffected |
