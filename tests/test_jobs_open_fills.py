...
def _write_quotes(paths, hhmm: str, ltp_by_symbol: dict[str, float]) -> None:
    ...
    rows = [_quote_row(sym, i + 1, ltp) for i, (sym, ltp) in enumerate(ltp_by_symbol.items())]
...
@freeze_time("2026-06-23 09:21:00+05:30")
...
def test_apply_skips_when_planner_funds_zero(paths) -> None:
    ...
    write_pending_entries(
        ...
        regime="NEUTRAL",
        entries=[
            PendingEntry(symbol="EXPENSIVE", atr_14=100.0, ml_score=0.6, ref_close=9_000_000.0)
        ],
    )
